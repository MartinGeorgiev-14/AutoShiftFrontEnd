...
const ConversationTop = ({ data }) => {
    ...
    const mainImg = data.response.listingCarDto.images.find(image => image.main === true)
    return (
        <div className="lg:h-[25%] z-1">
            <div className="h-full flex flex-col lg:justify-between">
                <h3 className="lg:p-3 lg:font-bold lg:text-lg">{data.response.firstName} {data.response.lastName}</h3>
                <hr className="lg:text-divider-component"></hr>
                    <div className="lg:flex lg:justify-start lg:gap-5 lg:p-3 lg:items-center lg:justify-between lg:bg-white lg:h-full">
                        ...
                        <div>
                            <p className="lg:font-bold">{data.response.listingCarDto.make} {data.response.listingCarDto.model}</p>
                            <p className="lg:font-bold lg:text-custom-blue">{data.response.listingCarDto.price} BGN</p>
                        </div>
                    </div>
                <hr className="lg:text-divider-component"></hr>
          </div>
